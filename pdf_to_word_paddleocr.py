...
class PDFtoWordConverterPaddleOCR:
    def __init__(self, root):
        ...
        self.lang_combobox['values'] = ["ch", "en", "ch+en"]
    ...
    def convert_with_paddleocr(self, pdf_path, docx_path, ocr_lang):
        ...
        # 初始化PaddleOCR（简化配置，避免兼容性问题）
        ocr = PaddleOCR(
            use_angle_cls=False,  # 禁用角度分类器以避免兼容性问题
            lang=ocr_lang       # 设置语言
        )
    ...
    if missing_libs:
        ...
        try:
            for lib in missing_libs:
                # 对于paddleocr，使用特定的安装命令
                if lib == 'paddleocr':
                    subprocess.check_call([sys.executable, "-m", "pip", "install", "--user", "paddlepaddle==2.6.2"])
                    subprocess.check_call([sys.executable, "-m", "pip", "install", "--user", "paddleocr>=2.6.0,<2.8.0"])
                else:
